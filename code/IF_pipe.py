...
from diffusers.pipelines.deepfloyd_if import IFPipelineOutput, IFImg2ImgPipeline
...
class IFPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def prepare_intermediate_images(
        ...
        image = image.repeat_interleave(num_images_per_prompt, dim=0).to(device)
...
if __name__ == "__main__":
    ...
    pipe.enable_model_cpu_offload()
    prompt = "A back and white image of the letter A"
    
    prompt_embeds, negative_embeds = pipe.encode_prompt(prompt, do_classifier_free_guidance=True)
    dtype = prompt_embeds.dtype
    prompt_embeds = torch.cat([negative_embeds, prompt_embeds])
    prompt_embeds = prompt_embeds.repeat_interleave(batch_size, 0)
    
    images = torch.randn(batch_size, 3, 224, 224) # random for now
    images = F.interpolate(images, size=pipe.unet.sample_size, mode="bilinear", antialias=True)
    # images = 0.18215 * images

    timestep = torch.randint(50, 950, (batch_size,), device=device)
    images.to(device)
    intermediate_images, eps = pipe.prepare_intermediate_images(
        images, timestep, batch_size, 1, dtype, device
    )

    z_in = torch.cat([intermediate_images] * 2) # ([10, 3, 64, 64])
    timestep_in = torch.cat([timestep] * 2)
    z_in = pipe.scheduler.scale_model_input(z_in, timestep_in)
    ...
    sigmas = (1 - pipe.scheduler.alphas_cumprod).to(device)
    w = alphas[timestep]**0.5 * sigmas[timestep]
    w = w[:, None, None, None]
