...
if __name__ == "__main__":
    ...
    for weight in torch.linspace(0.0, 1.0, steps=11):
        ...
        x = img.unsqueeze(0).permute(0, 3, 1, 2)  # HWC -> NCHW
        x = x.repeat(cfg.batch_size, 1, 1, 1)
        hyper_classification_loss = font_loss_rotate.forward(x)[0] + font_loss_normal.forward(x)[0]
        print(hyper_classification_loss.item() / cfg.batch_size)
        wandb.log({"hyper_classification_loss": hyper_classification_loss.item() / cfg.batch_size}, step=trial)
        trial += 1
